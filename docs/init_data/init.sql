...
#
CREATE TABLE IF NOT EXISTS `ctq`.`tctq_eth_address` (
...
#
CREATE TABLE IF NOT EXISTS `ctq`.`tctq_eth_transaction` (
...
# 初始化数据
insert INTO `ctq`.`tsys_config`(type, ckey, cvalue, updater, update_datetime, remark)
VALUES
  ('0','curBlockNumber','2130','code',now(),'下次从哪个区块开始扫描');